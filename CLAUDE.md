...
Homebrew refuses to load casks from untrusted third-party taps, but `Homebrew::Trust.explicitly_allowed?` treats the cask's full name appearing verbatim in `ARGV` as consent — so `brew install --cask <owner>/tap/go2shell` auto-taps, writes the trust entry and installs in one command. Verified with an empty `~/.homebrew/trust.json` and no tap. A bare `brew tap <owner>/tap` names no cask, gets no exemption, and fails with `invalid syntax in tap!` until `brew trust` runs — which is why the READMEs give the single fully-qualified install command and not a tap-then-install pair.
